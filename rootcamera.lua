		...
		if cameraSubject then
			if cameraSubject:IsA('Humanoid') then
				...
				if VRService.VREnabled and humanoidStateType == STATE_DEAD and cameraSubject == this.lastSubject then
					...
					if humanoidRootPart and humanoidRootPart:IsA('BasePart') and humanoidRootPart.Parent and humanoidRootPart.Parent:FindFirstChild("Head") then
						local subjectCFrame = GetRenderCFrame(humanoidRootPart.Parent.Head)
						local heightOffset = ZERO_VECTOR3
						if humanoidStateType ~= STATE_DEAD then
							heightOffset = Vector3.new(0,0,0)--cameraSubject.RigType == Enum.HumanoidRigType.R15 and R15HeadHeight or HEAD_OFFSET
						end
